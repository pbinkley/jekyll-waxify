  ...
  describe "#new" do
    context "when initialized with valid config hash from file" do
      it "runs without errors" do
        expect { Jekyll::Waxify::JekyllConfig.new(WAXIFY_FRAMEWORK, BUILD) }.not_to raise_error
      end
    end
  end
  ...
  describe "#add_cors" do
    context "when initialized with valid config hash from file" do
      it "adds a CORS section to new_config" do
        result = Jekyll::Waxify::JekyllConfig.new(WAXIFY_FRAMEWORK, BUILD)
        result.add_cors
  ...
  describe "#merge" do
    context "when initialized with valid config hash from file" do
      it "merges new_config into yaml" do
        result = Jekyll::Waxify::JekyllConfig.new(WAXIFY_FRAMEWORK, BUILD, { "waxified" => true })
        result.merge
